...
describe("Gemini Errors", () => {
  ...
  it("should handle 401 Unauthorized", async () => {
    const response = new Response(JSON.stringify({ error: { message: "API key not valid" } }), {
      status: 401
    });
  ...
  it("should handle 429 Rate Limit", async () => {
    const response = new Response(JSON.stringify({ error: { message: "Resource exhausted" } }), {
      status: 429
    });
  ...
  it("should handle 503 Service Unavailable", async () => {
    ...
    await expect(handleGeminiError(response, "test-model")).rejects.toThrow(
      ServiceUnavailableError
    );
  });

  it("should handle generic 500 error", async () => {
    const response = new Response(JSON.stringify({ error: { message: "Internal error" } }), {
      status: 500
    });
