...
`node-llm` provides a seamless, unified interface for interacting with multiple Large Language Model (NodeLLM) providers. Whether you are building a simple chat bot or a complex multi-modal agentic workflow, `node-llm` abstracts away the provider-specific complexities.
