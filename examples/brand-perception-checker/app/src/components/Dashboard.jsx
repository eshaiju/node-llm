    ...
    >
      <div className="grid-3">
        ...
        <div className="glass-panel">
          ...
          <div className="card-value">
            {report.ai_perception?.provider_disagreement === "Low"
              ? "High"
              : report.ai_perception?.provider_disagreement === "Medium"
                ? "Medium"
                : "Low"}
          </div>
          <div className="card-sub">
            Indicator across {report.ai_perception?.models?.length || 3} models
          </div>
        </div>
        <div className="glass-panel">
          <div className="card-title">
          ...
          <div className="card-sub">
            Trend: {report.market_perception?.sentiment_trend || "Neutral"}
          </div>
        </div>
      </div>
            ...
            <tbody>
              {report.ai_perception?.models?.map((m, i) => (
                <tr key={i}>
                  ...
                  <td>
                    {m.risk_mentions?.length > 0 ? (
                      m.risk_mentions.slice(0, 2).map((r, j) => (
                        <div key={j} className="list-item">
                          {r}
                        </div>
                      ))
        ...
        <div className="glass-panel col-4">
          ...
          <div style={{ marginBottom: "24px" }}>
            ...
            <div>
              ...
              {!report.market_perception?.paa?.length && (
            ...
            <div style={{ marginBottom: "24px" }}>
              {report.market_perception?.risk_signals?.length > 0 ? (
                report.market_perception.risk_signals.map((r, i) => (
                  <div key={i} className="list-item" style={{ color: "var(--danger)" }}>
                    {r}
                  </div>
                ))
              ) : (
      ...
      <div className="transparency-panel">
        ...
        <p style={{ color: "var(--text-muted)", fontSize: "0.8rem", lineHeight: 2 }}>
          This report is a deterministic aggregation of live data. No hidden scoring or black-box
          weighting.
          <br />
